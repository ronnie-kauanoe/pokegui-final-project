...
public abstract class Pokemon implements Comparable<Pokemon> {
   ...
   */
   ...
   public String toStringHTML() {
      ...
      s = "<html><div style='text-align: center;'>Species: " + species + "<br>";
      if (species.compareTo(name) != 0) {
         s = s + "Name: " + name + "<br>";
      }
      s = s + "Number: " +  df.format(number) + "<br>";
      s = s + "Height: " + height + "<br>";
      s = s + "Weight: " + weight + "<br>";
      s = s + "Type: " + type1;
      if (this.type2.length() > 0) {
         s = s + " | " + this.type2;
      }
  ...
  * @throws PokemonException Pokemon name cannot be blank
  */
   public void setName(String newName) throws PokemonException {
      if (newName.trim().equals("")) {
         throw new PokemonException("Error: Name cannot " 
            + "be empty or only contain blanks");
      } else {
         name = newName.trim();
      }
   }
   ...
   */  
   ...
   /**
   * @return result Returns true if compared Pokemon are exactly equal
   * @param other Other Pokemon being equated to 
   */
   public boolean equals(Pokemon other) {
      ...
      if (other.getNumber() == this.getNumber()
         ...
         && other.getCP() == this.getCP()
         && other.getFastAttack().equals(this.getFastAttack())
         && other.getSpecialAttack().equals(this.getSpecialAttack())) {
         result = true;
      } else {
         result = false;
      }
   ...
   /**
   * @param other Pokemon being compared to
   * @return result Compares Pokemon by number, HP, and CP
   */
   public int compareTo(Pokemon other) {
      int result = 0;
      int choice = PokePanel.getChoice();
      if (choice == 1) {
         if (this.equals(other)) {
            ...
            || other.getName() != other.getSpecies()) {
            result = this.getName().compareTo(other.getName());
         } else if (this.getHP() != other.getHP()) {
            result = this.getHP() - other.getHP();
         } else if (this.getCP() != other.getCP()) {
            result = this.getCP() - other.getCP();
         }
      } else if (choice == 2) {
         if (this.equals(other)) {
            result = 0;
         } else if (this.getSpecies() != other.getSpecies()) {
